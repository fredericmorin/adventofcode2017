lines = """dihjv (2158) -> gausx, ncdmp, hozgrub
...
# 		weight, _ = weight.split(')')
# 		childs = right.split(', ')
# 		programs[name] = dict(name=name, weight=int(weight), childs=childs)
# 	except:
# 		name, weight = line.split(' (')
# 		weight, _ = weight.split(')')
# 		programs[name] = dict(name=name, weight=int(weight), childs=[])

# # for program in programs.values():
# # 	print(program)

# def compute_cumulated_weight(program_name, depth=0):
# 	# print(str(depth) + ' ' + str(program_name))
# 	program = programs[program_name]
# 	childs = program['childs']
# 	if childs:
# 		program['cumulated_weight'] = 0
# 		for child_name in childs:
# 			program['cumulated_weight'] = program.get('cumulated_weight', 0) + compute_cumulated_weight(child_name, depth+1)
# 		return program['cumulated_weight']
# 	else:
# 		return program['weight']
# compute_cumulated_weight(root)

# def verify_program(program_name, depth=0):
# 	program = programs[program_name]
# 	childs = program['childs']
# 	if childs:
# 		oops = False
# 		weight = programs[childs[0]]['cumulated_weight']
# 		for child_name in childs:
# 			child = programs[child_name]
# 			if child['cumulated_weight'] != weight:
# 				oops = True
# 		if oops:
# 			print('---')
# 			for child_name in childs:
# 				child = programs[child_name]
# 				print('!!! ' + child['name'] + ' ' + str(child['cumulated_weight']))

# 		for child_name in childs:
# 			verify_program(child_name, depth+1)
# 	else:
# 		import sys
# 		sys.exit(0)
# verify_program(root)
