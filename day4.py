...
assert True == is_valid('aa bb cc dd ee')  # is valid.
assert False == is_valid('aa bb cc dd aa')  # is not valid - the word aa appears more than once.
assert True == is_valid('aa bb cc dd aaa')  # is valid - aa and aaa count as different words.
...
def is_valid2(input):
	...
	for word in words:
		...
		serialized = ''
		for key in sorted(d):
			serialized += key + str(d[key])
		letter_counts.append(serialized)
	# print input
	# print letter_counts
	# print len(letter_counts)
	# print len(set(letter_counts))

	if len(letter_counts) != len(set(letter_counts)):
		return False
	return True

assert True == is_valid2('abcde fghij')  # is a valid passphrase.
assert False == is_valid2('abcde xyz ecdab')  # is not valid - the letters from the third word can be rearranged to form the first word.
assert True == is_valid2('a ab abc abd abf abj')  # is a valid passphrase, because all letters need to be used when forming another word.
assert True == is_valid2('iiii oiii ooii oooi oooo')  # is valid.
assert False == is_valid2('oiii ioii iioi iiio')  # is not valid - any of these words can be rearranged to form any other word.
...
valid_passwords2 = [
	password for password in passwords
	if password and is_valid2(password)
]
