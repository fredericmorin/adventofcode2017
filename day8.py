instructions = """gug dec 188 if zpw >= 8
...
for line in instructions.split('\n'):
	parts = line.split(' ')
	if eval('registers.get(\'%s\', 0) %s %s' % (parts[4], parts[5], parts[6])):
		registers[parts[0]] = eval('%s %s %s' % (registers.get(parts[0], 0), '+' if parts[1] == 'inc' else '-', parts[2]))
